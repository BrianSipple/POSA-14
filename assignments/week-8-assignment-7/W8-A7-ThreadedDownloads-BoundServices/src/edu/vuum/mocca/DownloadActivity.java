 ...
 * applications from error-prone aspects of inter-process
 * communication.
 */
public class DownloadActivity extends DownloadBase {
	...
	DownloadCallback.Stub mDownloadCallback = new DownloadCallback.Stub() {
		...
		public void sendPath(final String imagePathname) throws RemoteException {
			...
			runOnUiThread(displayRunnable);  // return the displayRunnable's operation to the UI thread after its invoked by the service
		}
	};
	...
	public void runService(View view) throws RemoteException {
		...
		case R.id.bound_sync_button:
			...
			// onStart is where we already connected to the services -- creating the interfaces defined in AIDL -- so now it's time to call the correct proxy methods 
			
			String imagePath = mDownloadCall.downloadImage(uri);  
			mDownloadCallback.sendPath(imagePath);
			break;
